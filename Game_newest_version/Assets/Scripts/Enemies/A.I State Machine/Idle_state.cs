...
public class Idle_state : State{
    ...
    public override State Run_current_state(Enemy_manager enemy_manager){
        ...
        foreach( var character_collider in hit_colliders){
            if(character_collider != enemy_manager.gameObject.GetComponent<CapsuleCollider>()){
                ...
                if(viewable_angle > enemy_manager.minimum_detection_angle && viewable_angle < enemy_manager.maximum_detection_angle && Game_manager.Instance.player_movement.Is_enemy_to_lock_on_visible(enemy_manager.gameObject)){ // Is_enemy_to_lock_on_visible uses insidee tranform, the original script is attached to player so transform is player object not enemy object from which i call function
