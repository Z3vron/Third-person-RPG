...
public class Enemy_manager : MonoBehaviour
{
    ...
    #endregion
    [Header("Items/attacks - scriptable objects references")]
    ...
    private void Start() {
        ...
        instance_enemy_stats.enemy_HUD_to_this_stats = _instance_enemy_HUD.GetComponent<Enemy_HUD>();
    }
    void Update(){    
    ...
    private void Handle_HUD_enemies_lock_on_list(){
        if(_instance_enemy_HUD != null && Game_manager.Instance.player_movement != null){
            if(_instance_enemy_HUD.activeSelf){
    ...
    private void Handle_health_regen(){
        ...
        if(_health_regen_timer >= health_regen_delay)
            instance_enemy_stats.Health_regen(Time.deltaTime);  
    }
    private void Handle_death(){    
        animator.CrossFade("Death",0f,0);
        nav_mesh_agent.isStopped = true;
        Game_manager.Instance.player_info.player_stats.Add_exp(instance_enemy_stats.exp_reward);
        if(Game_manager.Instance.player_movement.enemies_lock_on.Contains(gameObject))
            Game_manager.Instance.player_movement.enemies_lock_on.Remove(gameObject);
        if(Game_manager.Instance.player_movement.enemies_lock_on.Count == 0 && Game_manager.Instance.player_info.locked_on_enemy)
            Game_manager.Instance.player_movement.Release_lock_on_enemy();
    ...
    private void Handle_recovery_time(){
        if(_current_recovery_time > 0 && animator.GetCurrentAnimatorStateInfo(0).IsName("Idle") ){
            _current_recovery_time -= Time.deltaTime;
        }
        if(performing_action && _current_recovery_time <=0){
    ...
    public void Set_recovery_time(float new_recovery_time){
        _current_recovery_time = new_recovery_time;
    }
}
